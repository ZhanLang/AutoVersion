...
template< class E >
std::basic_string< E >remove(std::basic_string< E > str,E s)
{
	std::basic_string<E>::size_type i = std::basic_string< E >::npos;
	while ((i = str.find(s)) != std::basic_string< E >::npos)
		str.erase(i, 1);
	return str;
}
...
std::basic_string< E > remove_non_lws( const E* s ) 
{
	...
	// Remove \r
	while( ( i = strResult.find( '\r' ) ) != std::basic_string< E >::npos )
		strResult.erase( i, 1 );

	// Remove \n
	while( ( i = strResult.find( '\n' ) ) != std::basic_string< E >::npos )
		strResult.erase( i, 1 );
...
template< class T >
std::basic_string<T> replace(std::basic_string<T> str, const std::basic_string<T>& old_value, const T* new_value)
{
	while (true)
	{
...
//×Ö·û´®·Ö¸î
template<class T>
std::vector<std::basic_string<T>> split(const std::basic_string<T>& str, const std::basic_string<T>& delimiters)
{  
